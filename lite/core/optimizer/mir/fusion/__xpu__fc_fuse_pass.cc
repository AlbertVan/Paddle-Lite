...
class XPUFcFuser : public FuseBase {
 public:
  ...
  void BuildPattern() override {
    ...
    auto mul_input_check = [](const Node* node) -> bool {
      ...
      return (mul_y_shape.size() == 2 && 
        (!op_desc.HasAttr("transpose_X") || !op_desc.GetAttr<bool>("transpose_X")) &&
        (!op_desc.HasAttr("trans_x") || !op_desc.GetAttr<bool>("trans_x")));
    };
    auto* mul = OpNode("mul", mul_type_)->AsIntermediate()->assert_node_satisfied(mul_input_check);
  ...
  void InsertNewNode(SSAGraph* graph, const key2nodes_t& matched) override {
    ...
    if (mul_type_ == "mul") {
      op_desc.SetAttr("in_num_col_dims", op_info->GetAttr<int>("x_num_col_dims"));
