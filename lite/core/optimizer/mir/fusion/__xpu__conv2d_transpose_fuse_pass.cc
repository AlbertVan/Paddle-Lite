...
class XPUConv2dTransFuser : public FuseBase {
 public:
  ...
  void InsertNewNode(SSAGraph* graph, const key2nodes_t& matched) override {
    ...
    op_desc.SetAttr<std::vector<int>>("paddings", conv_op_desc->GetAttr<std::vector<int>>("paddings"));
    op_desc.SetAttr<std::vector<int>>("strides", conv_op_desc->GetAttr<std::vector<int>>("strides"));
    op_desc.SetAttr<std::vector<int>>("dilations", conv_op_desc->GetAttr<std::vector<int>>("dilations"));
      ...
      for (auto with_conv_bias : {true, false}) {
        for (auto with_bn : {true, false}) {
          for (auto act_type : {"relu"}) {
            fusion::XPUConv2dTransFuser fuser(conv_type, act_type, with_conv_bias, with_bn);
            fuser(graph.get());
          }
        }
      }
...
REGISTER_MIR_PASS(__xpu__conv2d_transpose_fuse_pass, paddle::lite::mir::XPUConv2dTransFusePass)
