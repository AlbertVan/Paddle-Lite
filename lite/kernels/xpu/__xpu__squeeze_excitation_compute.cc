...
using squeeze_excitation_block_fp32 =
    paddle::lite::kernels::xpu::XPUSqueezeExcitationCompute<float, PRECISION(kFloat)>;
using squeeze_excitation_block_fp16 =
    paddle::lite::kernels::xpu::XPUSqueezeExcitationCompute<float16, PRECISION(kFP16)>;
...
REGISTER_LITE_KERNEL(__xpu__squeeze_excitation_block,
    ...
    .BindInput("Filter", {LiteType::GetTensorTy(TARGET(kHost))})
    .BindInput("Bias", {LiteType::GetTensorTy(TARGET(kXPU))})
    .BindInput("Branch", {LiteType::GetTensorTy(TARGET(kXPU), PRECISION(kFP16))})
